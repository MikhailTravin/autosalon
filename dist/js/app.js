 ...
 */
    ...
    (() => {
        ...
        function select_select() {
            ...
            optionMenus.forEach((optionMenu => {
                ...
                if (optionMenu) {
                    ...
                    if ("" !== sBtntext.value) sBtntext.classList.add("filled"); else sBtntext.classList.remove("filled");
                    options.forEach((option => {
                        ...
                        if (selectClear) selectClear.addEventListener("click", (function(e) {
                            optionMenu.classList.remove("_active");
                        }));
                    }));
                    if (selectClose) selectClose.addEventListener("click", (function(e) {
                        optionMenu.classList.remove("_active");
        ...
        function rangeInit() {
            ...
            if (productCardRangeTerm) nouislider.create(productCardRangeTerm, {
                start: 5,
                behaviour: "snap",
                connect: [ true, false ],
                pips: {
        ...
        document.querySelectorAll(".products").forEach((n => {
            ...
            function catalogSliderInit() {
                if (!productsSlider) {
                    new core(n.querySelector(".products__slider"), {
...
***************************************************************************** */
        ...
        function indents() {
            ...
            if (productCard) productCard.style.paddingTop = hHeader + "px";
        ...
        if (filterClear) filterClear.addEventListener("click", (function(e) {
        ...
        const maxActive = 2;
        if (compareCheckboxes) compareCheckboxes.forEach((block => {
        ...
        if (track) {
            ...
            function setupTrack(slides) {
                track.innerHTML = "";
                slides.slice().reverse().forEach((slide => track.prepend(slide.cloneNode(true))));
                slides.forEach((slide => {
                    for (let i = 0; i < cloneCount; i++) track.appendChild(slide.cloneNode(true));
                }));
                const slideMargin = parseFloat(getComputedStyle(slides[0]).marginRight);
                const startPosition = -slides.reduce(((acc, slide) => acc + slide.offsetWidth + slideMargin), 0);
            ...
            document.querySelector(".fortune-popup__button").addEventListener("click", (() => {
                ...
                requestAnimationFrame((() => {
                    ...
                    const containerWidth = content.clientWidth - parseFloat(getComputedStyle(content).paddingLeft) - parseFloat(getComputedStyle(content).paddingRight);
                    const targetIndex = Math.floor(Math.random() * currentSlides.length);
                    const containerCenter = containerWidth / 2;
                    let targetPosition = currentPosition;
                    currentSlides.forEach(((_, i) => {
                        if (i <= targetIndex) targetPosition += slideWidth + slideMargin;
                    }));
                    targetPosition -= slideWidth / 2 + containerCenter;
                    const totalDistance = targetPosition + 3 * currentSlides.reduce(((acc, slide) => acc + slide.offsetWidth + slideMargin), 0);
                    ...
                    track.addEventListener("transitionend", (() => {
                        ...
                        const trackWidth = currentSlides.reduce(((acc, slide) => acc + slide.offsetWidth + slideMargin), 0);
                        ...
                        Array.from(track.children).some(((slideEl, index) => {
                            ...
                            if (slideRect.left <= centerPosition && slideRect.right >= centerPosition) {
                                ...
                                const originalSlide = initialSlides.find((s => s.dataset.id === currentSlides[originalIndex].dataset.id));
                                selectedData = {
